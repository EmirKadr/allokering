        ...
        for hib in hib_orders:
            ...
            for _, zrow in hib_zone_rows.iterrows():
                ...
                if mval:
                    ...
                    missing_multi = True
        # multi_mismatch om olika multi‑nummer, eller saknad, eller zonflagga
...
class App(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        # Fält för Orderöversikt (CSV)
        ttk.Label(indata_frame, text="Orderöversikt (CSV):").grid(row=4, column=0, sticky="w", padx=4, pady=4)
        self.overview_var = tk.StringVar()
        self.overview_entry = ttk.Entry(indata_frame, textvariable=self.overview_var)
        self.overview_entry.grid(row=4, column=1, sticky="ew", padx=4)
        ttk.Button(indata_frame, text="Bläddra...", command=self.pick_overview).grid(row=4, column=2, padx=4)
        ...
        if TkinterDnD and DND_FILES:
            ...
            # Bind även orderöversikten till drag-and-drop
            try:
                bind_drop(self.overview_entry, self.overview_var)
            except Exception:
                pass

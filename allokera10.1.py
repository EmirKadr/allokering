...
allokera10.1.py
---------------
Denna version (10.1) bygger vidare på tidigare versioner och lägger till
förbättrad HIB‑koppling, robust kolumnmatchning och ett förenklat GUI för filuppladdning:

  * Multi‑reglerna (fel zon, saknad multi) gäller bara när det finns mer än
    en HIB‑order per kundnummer. Med endast en HIB‑order sätts inte multi.
  * Instruktioner för ändringsordning skrivs nu ut i loggen efter körning
    av HIB‑kopplingen och finns även i exportfilen.
  * Kolumnmatchningen i orderöversikten är robust mot olika ordning och namn.
  * Indata‑filvalet använder nu statusikoner (grön check, grå minus och röd
    borttagningsknapp) istället för textfält. Dra‑och‑släpp‑zonen kan även
    klickas för att välja flera filer samtidigt.

Version 10.1 inkluderar även fixar för filuppladdning där statusikoner
initialiseras korrekt innan GUI‑widgets skapas och programtiteln uppdateras.
"""
...
APP_TITLE = "Buffertpallar → Order-allokering (GUI) — 10.1"
...
def compute_hib_koppling(
    ...
    for kund_nr, kund_df in ov.groupby("Kund nr"):
        ...
        for _, row in store_df.iterrows():
            ...
            # Spara första giltiga
            if base_candidate is None:
                base_candidate = row
                base_date = odate
            else:
                # Jämför datum, ta den äldsta (lägst)
                try:
                    # Försök tolka som datum; om tolkningen misslyckas används strängjämförelse
                    d_new = pd.to_datetime(odate, errors="coerce")
                    d_old = pd.to_datetime(base_date, errors="coerce")
                    if (pd.isna(d_old) and not pd.isna(d_new)) or (
                        not pd.isna(d_old) and not pd.isna(d_new) and d_new < d_old
                    ):
                        base_candidate = row
                        base_date = odate
                    elif pd.isna(d_new) and pd.isna(d_old) and odate < base_date:
                        base_candidate = row
                        base_date = odate
                except Exception:
                    if odate < base_date:
        ...
        if base_candidate is None:
            ...
            continue
        # Hämta referensdata från butiken
        ...
        # Bestäm multi‑mismatch för kunden
        # Samla multi‑nummer för HIB‑ordrar i zon F (i orderöversikten)
        multi_vals: set[str] = set()
        missing_multi = False
        for hib in hib_orders:
            h_ord = hib["ordernr"]
            # Hitta alla rader i kund_df (som redan filtrerats) för denna order där zon är F
            hib_zone_rows = kund_df[(kund_df["Ordernr"] == h_ord) & (kund_df["Zon"].astype(str).str.strip().str.upper() == "F")]
            if hib_zone_rows.empty:
                # ingen rad i zon F => behandla som saknad multi
                missing_multi = True
                continue
            for _, zrow in hib_zone_rows.iterrows():
                ...
                if mval:
                    ...
                    missing_multi = True
        # Bedöm om multi behöver sättas till "MULTI".
        # Om det finns fler än en HIB‑order för kunden gäller följande:
        #   - Fel zon (minst en rad ej F/H/R)
        #   - Saknar multi (inget multi‑nummer i någon zon F‑rad)
        #   - Olika multi‑nummer
        # Om endast en HIB finns gäller endast regeln om olika multi‑nummer.
        if len(hib_orders) > 1:
            # Om flera HIB‑ordrar: kontrollera zonflagga, saknad multi eller olika multi‑nummer
            multi_mismatch = zone_flag or missing_multi or (len(multi_vals) != 1)
        else:
            # Endast en HIB‑order: inga regler triggar multi
        ...
        for hib in hib_orders:
            ...
            # Sändningsnummer och orderdatum jämförs mot butiksordern (basen)
            cur_ship = str(h_row["Sändningsnr"]).strip()
            cur_date = str(h_row["Orderdatum"]).strip()
            if cur_ship != base_ship:
                ship_update = base_ship
            if cur_date != base_odate:
                date_update = base_odate
            multi_update = "MULTI" if multi_mismatch else ""
...
class App(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        status_orders = tk.Label(indata_frame, text="–", fg="#666666", bg="#e0e0e0", width=3, font=("Arial", 12, "bold"))
        ...
        status_buffer = tk.Label(indata_frame, text="–", fg="#666666", bg="#e0e0e0", width=3, font=("Arial", 12, "bold"))
        ...
        status_automation = tk.Label(indata_frame, text="–", fg="#666666", bg="#e0e0e0", width=3, font=("Arial", 12, "bold"))
        ...
        status_item = tk.Label(indata_frame, text="–", fg="#666666", bg="#e0e0e0", width=3, font=("Arial", 12, "bold"))
        ...
        status_overview = tk.Label(indata_frame, text="–", fg="#666666", bg="#e0e0e0", width=3, font=("Arial", 12, "bold"))
        ...
        self.drop_zone.grid(row=5, column=0, columnspan=3, sticky="nsew", padx=4, pady=8)
        ...
        self.reset_cache_btn.grid(row=0, column=6, padx=4)
    ...
    def update_file_status_icons(self) -> None:
        ...
        try:
            for ft, (lbl, btn) in self.file_status_widgets.items():
                ...
                if path:
                    # fil har valts: grön bakgrund med vit text
                    lbl.config(text="✓", fg="white", bg="#28a745")
                    # enable remove button for tk.Button
                    btn.config(state="normal")
                else:
                    # ingen fil: grå bakgrund och diskret text
                    lbl.config(text="–", fg="#666666", bg="#e0e0e0")
    ...
    def run_koppla(self) -> None:
        ...
        try:
            ...
            return
        # Spara resultat
        self.last_koppla_df = changes_df.copy()
        # Om inga ändringar behövs, meddela användaren och stäng av öppna‑knappen
        if changes_df is None or changes_df.empty:
            self.open_koppla_btn.config(state="disabled")
            messagebox.showinfo(APP_TITLE, "Inga HIB‑ordrar behöver ändras.")
            return
        ...
        try:
            ...
            # Lägg till instruktioner i loggen för hur ändringarna ska utföras
            instr_lines = [
                "\nInstruktion:",
    ...
    def open_koppla_in_excel(self) -> None:
        ...
        """
        if isinstance(self.last_koppla_df, pd.DataFrame) and not self.last_koppla_df.empty:
            try:
                instr_lines = [
                    "Ändras i följande ordning",
                    "1. Ordernummer",
                ...
                sheets = {
                    "Ändringar": self.last_koppla_df.copy(),
                    "Instruktion": instructions_df,
                }
